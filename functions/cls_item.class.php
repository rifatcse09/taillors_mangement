...
class cls_item {
    ...
    public function insert_item($category, $item_name, $item_code, $size, $description, $user_id, $fileName) {
        ...
        if($row_result>0)
        {
            ...
            $result = DB::query("INSERT INTO `tbl_items` (`id`, `item_code`, `cat_id`, `item_name`, `size`, `photo`, `description`, `saved_by`, `saved_date`) VALUES (NULL, '$item_code', '$category', '$item_name', '$size', '$fileName', '$description', '$user_id', '$datetime')");
//            $result = DB::query("insert into tbl_items(item_code,cat_id,item_name,size,photo,description,saved_by,saved_date) values ('$item_code','$category','$item_name','$size',$fileName','$description','$user_id','$datetime')");
    ...
    public function viewitemAllwith_cat() {
        $restul = DB::query("select tbl_items.*,tbl_category.cat_name from tbl_items inner join tbl_category on tbl_category.id = tbl_items.cat_id where tbl_items.status = '1' order by tbl_items.item_name asc");
        return $restul;
    ...
    public function sale_item_show($item_id) {
        $restul = DB::query("SELECT a.*,b.cat_name,(select price from tbl_item_price where item_id='$item_id' order by id desc limit 1 ) as price  FROM tbl_items as a join tbl_category as b   where a.cat_id=b.id  and a.status = '1' and a.id='$item_id'");
        return $restul;
    }
    //
    //item view all//
    ...
    public function update_item($item_id, $category, $item_name, $item_code, $size, $description, $user_id, $fileName) {
        ...
        if($row_count>0)
        {
            ...
            if ($fileName == '') {
                ...
                $result = DB::query("update tbl_items set item_code='$item_code',cat_id='$category',item_name='$item_name',size='$size',description='$description',saved_by='$user_id',saved_date='$datetime' where id='$item_id'");

            } else {
                $result = DB::query("update tbl_items set item_code='$item_code',cat_id='$category',item_name='$item_name',size='$size',description='$description',photo='$fileName',saved_by='$user_id',saved_date='$datetime' where id='$item_id'");
            }
    ...
    public function item_price_info($item_id)
{
    ...
    $result = DB::query("
            SELECT tbl_items.*,
            IFNULL((select price from tbl_item_price
    ...
    public function item_price_add($item_id, $slaes_price, $saved_by)
    {
        $cls_datetime = new cls_datetime();
        $datetime = $cls_datetime->datetime();
        $exat_date = $cls_datetime->exat_date();

        $item_price = $this->item_price($item_id);
        if($item_price->num_rows>0){
            $result = DB::query("UPDATE `tbl_item_price` SET `price` = '$slaes_price' WHERE `tbl_item_price`.`item_id` = $item_id");
        }else{
            $result = DB::query("insert into tbl_item_price(id,item_id,price,saved_by,saved_date) values (NULL,'$item_id','$slaes_price','$saved_by','$datetime')");
        }
    ...
    public function item_price($item_id)
    {
        $result=DB::query("SELECT * FROM `tbl_item_price` where item_id = '$item_id'");
        return $result;
    }

    public function item_info($item_id)
    {
        $result=DB::query("select a.size,b.available_stock from tbl_items as a join tbl_stock as b on a.id = b.item_id where a.id = '$item_id'");
        return $result;
    }
