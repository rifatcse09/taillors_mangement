...
class cls_item {
    ...
    public function insert_item($category, $item_name, $item_code, $size, $unit, $description, $user_id) {
        ...
        if($row_result>0)
        {
            ...
            $result = DB::query("insert into tbl_items(item_code,cat_id,item_name,unit,description,saved_by,saved_date) values ('$item_code','$category','$item_name','$unit','$description','$user_id','$datetime')");
    ...
    public function pur_item_show($item_id) {
        $restul = DB::query("SELECT a.*,b.cat_name,IFNULL((select price from tbl_item_price 
                where item_id=$item_id 
                order by id desc limit 1),0.00) as price
                FROM tbl_items as a join tbl_category as b on a.cat_id=b.id and a.id='$item_id' and a.status = '1'");
        return $restul;
    }
...
//        $restul = DB::query("SELECT a.*,b.cat_name,(select available_stock from tbl_stock where item_id='$item_id')as stock,(select price from tbl_item_price where item_id='$item_id' order by id desc limit 1 ) as price,IFNULL((select  discount from tbl_item_price where item_id='$item_id' and CURDATE() between promo_from and promo_to order by id desc limit 1 ),0.00)as discount FROM tbl_items as a join tbl_category as b   where a.cat_id=b.id  and a.status = '1' and a.id='$item_id'");
    ...
    public function  viewitemby_name($item_name,$size) {
        $result = DB::query("select * from tbl_items where item_name = '$item_name'  and status = '1'");
        return $result;
    }
    ...
    public function update_item($item_id, $category, $item_name, $item_code, $size, $unit, $description, $user_id) {
